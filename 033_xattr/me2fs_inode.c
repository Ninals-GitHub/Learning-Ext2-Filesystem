						   ...
						   int create )
{
	...
	/* NOW allocate blocks														*/
	/* ------------------------------------------------------------------------ */
	if( S_ISREG( inode->i_mode ) && ( !mi->i_block_alloc_info ) )
	{
	...
	/* ------------------------------------------------------------------------ */
	count	= blocksToAllocate( partial,
								indirect_blks,
								maxblocks,
								blocks_to_boundary );
...
found:
	...
	err = count;
	DBGPRINT( "<ME2FS>%s:found block=%lu, num=%d\n",
			  __func__,
			  ( unsigned long )le32_to_cpu( chain[ depth - 1 ].key ), count );
						...
						Indirect *branch )
{
	...
	num = allocBlocks( inode, goal, indirect_blks, *blks, new_blocks, &err );
						  ...
						  int blks )
{
	...
	*where->p = where->key;
	...
	/* ------------------------------------------------------------------------ */
	/* update the most recently allocated logical & physical block in			*/
	/* i_block_alloc_info, to assist find the proper goal block for next alloc	*/
	/* ------------------------------------------------------------------------ */
	if( block_i )
	{
		block_i->last_alloc_logical_block	= block + blks - 1;
		block_i->last_alloc_physical_block	= le32_to_cpu( where[ num ].key ) +
											  blks - 1;
	}
...
findGoal( struct inode *inode, long block, Indirect *partial )
{
	...
	/* ------------------------------------------------------------------------ */
	/* try the heuristic for sequential allocation, failing that at least		*/
	/* try to get decent locality												*/
	/* ------------------------------------------------------------------------ */
...
/*
==================================================================================
	Function	:blocksToAllocate
	Input		:Indirect *branch
				 < indirect information >
				 int k
				 ...
				 int blocks_to_boundary
				 < boundary in indirect blocks >
	Output		:void
	Return		:unsigned long
				 < blocks to allocate >

	Description	:lookup the block map and count the number of direct blocks
				 need to be allocated for the given branch
==================================================================================
*/
static unsigned long
blocksToAllocate( Indirect *branch,
						...
						int *err )
{
	...
	/* ------------------------------------------------------------------------ */
	/* here trying to allocate the requested multiple blocks at once,			*/
	/* on a best-effort basis.													*/
	/* to build a branch, should allocate blocks for the indirect blocks		*/
	/* (if not allocated yet), and at lease the first direct block of this		*/
	/* branch. that's the minimum number of blocks need to allocate (required)	*/
	/* ------------------------------------------------------------------------ */
	...
	while( 1 )
	{
		...
		/* -------------------------------------------------------------------- */
		current_block = me2fsNewBlocks( inode, goal, &count, err );
